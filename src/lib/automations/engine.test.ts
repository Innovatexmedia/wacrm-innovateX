  ...
  function resolve(ops: {
    ...
    if (table === "conversations") {
      if (type === "insert") {
        ...
        if (state.conversationInsertError) {
          return { data: null, error: state.conversationInsertError };
        }
        return { data: { id: "conv-new-1" }, error: null };
      }
      // Find query (no .single()/.maybeSingle() — resolves via `then`,
      // so `data` is an array like the real client returns without one).
...
describe("tag_added — conversation policy", () => {
  ...
  it("creates a conversation and sends when the contact has none yet", async () => {
    tagOutreachFixture();
    h.state.existingConversation = null;
    ...
    expect(h.state.conversationInserts[0]).toMatchObject({
      account_id: ACCOUNT,
      contact_id: "c1",
      // Audit column: the automation's own author, not the caller.
      user_id: "u1",
    });
    const withStatus = h.state.logUpdates.filter((u) => "status" in u);
    expect(withStatus.at(-1)).toMatchObject({ status: "success" });
  ...
  it("reuses an existing conversation instead of creating a new one", async () => {
    tagOutreachFixture();
    h.state.existingConversation = { id: "conv-existing" };
    ...
    expect(h.state.conversationInserts).toHaveLength(0);
    const withStatus = h.state.logUpdates.filter((u) => "status" in u);
    expect(withStatus.at(-1)).toMatchObject({ status: "success" });
  });

  it("records a clear failed step when conversation creation itself errors", async () => {
    tagOutreachFixture();
    h.state.existingConversation = null;
    h.state.conversationInsertError = { message: "db unavailable" };
    ...
    expect(h.state.logUpdates).toContainEqual(expect.objectContaining({
      status: "failed",
      error_message: "cannot resolve conversation: db unavailable",
    }));
  });
});
