 ...
 * manual engine POSTs. If none exists yet — e.g. a contact created via
 * the public API or CSV import, who has never messaged in — create one
 * rather than failing the step. Mirrors findOrCreateConversation in the
 * WhatsApp webhook handler; kept separate to avoid risking that working
 * path.
 */
async function resolveConversationId(args: ExecuteArgs): Promise<string> {
  ...
  const db = supabaseAdmin()
  // Oldest-first + single row, same reasoning as the webhook's lookup:
  // avoids .single()'s "0 or 2+ rows both error" trap compounding into
  // duplicate conversations under a race.
  ...
  const { data: created, error: createError } = await db
    ...
    .insert({
      account_id: args.automation.account_id,
      // Audit-only column, not consulted for tenancy (see meta-send.ts) —
      // the automation's own author is a valid owner for a conversation
      // the automation itself opened.
      user_id: args.automation.user_id,
      contact_id: args.contactId,
    })
    .select('id')
  ...
  // Lost a race: something else (another automation step, a concurrent
  // inbound) created the conversation between our lookup and insert.
  // Re-resolve rather than fail the step.
  ...
  throw new Error(`cannot resolve conversation: ${createError.message}`)
}
