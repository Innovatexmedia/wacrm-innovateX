...
The public API lets you drive your InnovateX CRM account from your own
...
```
Authorization: Bearer innovatexmedia_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** InnovateX CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer innovatexmedia_live_xxx"
```
...
Rather than polling, register an endpoint and InnovateX CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; InnovateX CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer innovatexmedia_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/innovatex", "events": ["message.received"] }'
...
Headers: `X-InnovateX-Event`, `X-InnovateX-Webhook-Id`, and `X-InnovateX-Signature`.

### Verifying the signature

`X-InnovateX-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages InnovateX CRM stores (inbox + API sends), not broadcast-only
