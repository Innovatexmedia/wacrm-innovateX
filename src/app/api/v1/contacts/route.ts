...
export async function POST(request: Request) {
  try {
    ...
    // Fire new_contact_created for genuinely new rows only — parity with
    // the WhatsApp webhook's dispatch (which also gates on `wasCreated`).
    // Tags are set above, before this, so a condition step checking
    // tag_presence sees tags passed in this same request. Awaited (not
    // fire-and-forget): on serverless this handler can be frozen right
    // after the response is sent, so a detached dispatch could die
    // mid-run — same reasoning as the webhook route.
    if (created) {
      await runAutomationsForTrigger({
        ...
        contactId: id,
        context: {},
      }).catch((err) => console.error('[api/v1/contacts] automation dispatch failed:', err));
    }
