...
interface BroadcastPayload {
  ...
  /**
   * Per-URL-button {{1}} value, keyed by the button's index in
   * `template.buttons`. Unlike body placeholders, Meta's send API has
   * no silent fallback for a URL button's variable — omitting it
   * fails the send outright, so this is resolved and sent as
...
/**
 * Resolve a single button-variable value for one contact — same
 * source rules as `resolveVariables` (static / contact field / custom
 * field), but a scalar instead of an ordered array since a URL button
 * carries exactly one {{1}}.
 */
function resolveButtonVariable(
        ...
        const apiRecipients = batch
          ...
          .map((r) => {
            ...
            if (hasButtonVariables) {
              for (const [indexStr, mapping] of Object.entries(buttonVariables)) {
                buttonParams[Number(indexStr)] = resolveButtonVariable(
                  mapping,
                  contact,
                  customValueIndex.get(contact.id)
                );
            ...
            const messageParams =
              (isMediaHeader && headerMediaUrl) || hasButtonVariables
                ? {
                    ...(isMediaHeader && headerMediaUrl ? { headerMediaUrl } : {}),
                    ...(hasButtonVariables ? { buttonParams } : {}),
                  }
                : undefined;
