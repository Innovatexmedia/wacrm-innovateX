...
//
// Every delivery carries an `X-InnovateX-Signature` header so receivers
// can verify the request really came from InnovateX CRM and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-InnovateX-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-InnovateX-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so an integration between two InnovateX CRM accounts
