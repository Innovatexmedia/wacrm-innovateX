...
interface Step3Props {
  ...
  /**
   * Per-URL-button variable mapping, keyed by the button's index in
   * `template.buttons`. Meta allows at most one {{1}} per URL button
   * (validated at template-creation time), so each entry maps that
   * one slot — same VariableMapping shape as body variables, reused
   * so "static text" / "contact field" / "custom field" all work the
   * same way here.
   */
  buttonVariables: Record<number, VariableMapping>;
  onButtonVariablesUpdate: (vars: Record<number, VariableMapping>) => void;
  onNext: () => void;
  onBack: () => void;
}
...
export function Step3Personalize({
  ...
  // URL buttons carrying a {{1}} in their url need a per-send value —
  // Meta's send API rejects the message outright without one (unlike
  // body placeholders, there's no silent fallback). Template creation
  // only allows a URL button exactly one variable, always numbered
  // {{1}}, so button index alone is enough to key this map.
  const urlButtonsNeedingValue = useMemo(() => {
    return (template.buttons ?? [])
      .map((button, index) => ({ button, index }))
      .filter(
        (entry): entry is { button: Extract<TemplateButton, { type: 'URL' }>; index: number } =>
          entry.button.type === 'URL' && entry.button.url.includes('{{1}}')
      );
  }, [template.buttons]);
  ...
  const unmappedButtonIndexes = useMemo(() => {
    return urlButtonsNeedingValue
      .map(({ index }) => index)
      .filter((index) => {
        const mapping = buttonVariables[index];
        return !mapping || !mapping.value?.trim();
      });
  }, [urlButtonsNeedingValue, buttonVariables]);
  ...
  function updateButtonVariable(index: number, patch: Partial<VariableMapping>) {
    const current = buttonVariables[index] ?? { type: 'static' as VariableType, value: '' };
    onButtonVariablesUpdate({
      ...buttonVariables,
      [index]: { ...current, ...patch },
    });
  ...
  return (
    <div className="space-y-6">
      ...
      {placeholders.length === 0 && !mediaHeaderType && urlButtonsNeedingValue.length === 0 ? (
      ...
      {urlButtonsNeedingValue.length > 0 && (
        <div className="space-y-4">
          {urlButtonsNeedingValue.map(({ button, index }) => {
            const mapping = buttonVariables[index] ?? { type: 'static', value: '' };
              ...
              >
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t('personalize.urlButtonLabel', { text: button.text })}
                  </span>
                </div>
                  ...
                  <div>
                    ...
                    {mapping.type === 'static' ? (
                      <Input
                        value={mapping.value}
                        onChange={(e) =>
                          updateButtonVariable(index, { value: e.target.value })
                        }
                        placeholder={t('personalize.enterValue')}
                        className="border-border bg-muted text-foreground placeholder:text-muted-foreground"
                      />
                      ...
                      <Select
                        value={mapping.value || undefined}
                        onValueChange={(val) =>
                          updateButtonVariable(index, { value: val || '' })
                        }
                      >
                        <SelectTrigger className="w-full border-border bg-muted text-foreground">
                          <SelectValue placeholder={t('personalize.selectContactField')} />
      ...
      {(unmappedKeys.length > 0 || unmappedButtonIndexes.length > 0) && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
          ...
          {unmappedButtonIndexes.length > 0 && (
            <p className={unmappedKeys.length > 0 ? 'mt-1' : undefined}>
              {t('personalize.unmappedButtonWarning', { count: unmappedButtonIndexes.length })}
            </p>
          )}
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        ...
        <Button
          onClick={onNext}
          disabled={unmappedKeys.length > 0 || unmappedButtonIndexes.length > 0 || headerMediaError !== null}
